...
requirements = (OpSys == "macOS")                       either Mac mini
```

**Naming `OpSys` is mandatory to reach either Mac mini, and forgetting it fails
...
| the two Mac minis | `mac-mini-m4` | `Apple Mac mini M4` |

**The Mac minis' `Arch` column reads `X86_64`, and the machines are arm64.** This
...
are translated, and they are idle by comparison. A Condor job on those machines
...
match on for those hosts, however wrong it looks, and `HostBoardClass` is the
