\chapter{Roadmap and open risks}
\label{ch:roadmap}
...
\section{Phase 4 is blocked on a storage decision}
...
\section{Heterogeneity, deferred to phase 5}
...
GPUs & \kn{condor\_gpu\_discovery}, \kn{DetectedGPUs}, \kn{request\_gpus}. Jetson iGPU detection under L4T is not reliably clean --- verify, do not assume \\
\bottomrule
\end{tabularx}
...
\midrule
Two unroutable subnets in one pool & Solved via \kn{PRIVATE\_NETWORK\_*} with a derived, not configured, name (Chapter~\ref{ch:net}) \\
...
Suspend, memory eviction, admission closure & Verified under load (Chapter~\ref{ch:validation}) \\
\bottomrule
\end{tabularx}
...
\section{The next three things}

\begin{enumerate}
  \item \textbf{Exercise disk-pressure eviction.} It is the only branch of
        \kn{EVICT\_PRESSURE} never observed firing, and Chapter~\ref{ch:failure}
  ...
  \item \textbf{Decide the isolation model.} It is cheap now and expensive later,
        ...
        each other --- which, now that the pool runs on colleagues' own desktops
        rather than spare hardware, is no longer a hypothetical question.
\end{enumerate}
