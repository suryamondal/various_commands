...
4b & Hosted kernels, for the flagship and GPUs only & not started, needs shared storage \\
...
\begin{table}[H]
\centering
\begin{tabularx}{\textwidth}{L{0.22\textwidth} L{0.16\textwidth} L{0.24\textwidth} Y}
\toprule
& \textbf{What moves} & \textbf{What it needs} & \textbf{When it wins} \\
\midrule
Notebook submits & jobs & nothing pool-side & almost always here \\
Kernel hosted in the pool & the kernel process & shared storage, machines safe for interactive work & only when a pool machine is larger than the user's own \\
\bottomrule
\end{tabularx}
\caption{Two architectures, routinely conflated.}
\end{table}

A notebook kernel is \textbf{one process on one machine}. Spawning it on a
four-core mini-PC makes it slower than a laptop with twelve threads, and no
distribution occurs at all. Hosting the kernel is a placement decision rather
than a parallelism one, and in this fleet it pays on exactly one machine, the
sixteen-thread worker, and later on a Jetson for its GPU.

The architecture that uses a pool of small machines leaves the kernel where it
already is and fans the \emph{work} out. The kernel becomes a coordinator,
cheap and single-threaded, while parallelism lives in many short jobs.
...
Shared storage for hosted kernels & \textbf{Required for phase 4b only.} Phase 4 needs none (\S\ref{sec:notebooks}) \\
