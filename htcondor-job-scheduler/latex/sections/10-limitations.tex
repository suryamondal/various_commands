...
5 & ARM workers & \textbf{done} (Jetson, aarch64) \\
5a & GPU discovery on L4T & \textbf{done}, needs \kn{condor} in \kn{video,render} \\
...
\begin{keypoint}
Adding the first non-x86 worker changed nothing about the policy. The Jetson
runs the same owner policy, the same \kn{STARTD\_CRON} job and the same
measured-memory override, unmodified, and Ubuntu ships the pool's exact Condor
version for arm64. The two things that did differ were about \emph{size}: the
standard \kn{RESERVED\_DISK} exceeded that machine's entire free space, which
would have left it advertising cores while refusing every job, and it needed no
interface pin because it is dual-homed with no VPN interface.
\end{keypoint}
...
Jetson GPU discovery & \textbf{Works.} Requires \kn{condor} in groups \kn{video} \emph{and} \kn{render}. The failures are \kn{NvRmMemInitNvmap denied} with neither and \kn{cuInit 801} with \kn{video} alone, so neither names permissions after the first \\
A GPU job running as \kn{nobody} & \textbf{Works} after \kn{usermod -aG video,render nobody}, verified end to end. No condor restart needed: the starter forks a fresh process per job \\
\kn{request\_gpus} is advisory on the Jetson & \textbf{Accepted.} Access belongs to the account every job shares, so a job can use the GPU without reserving one. Consistent with \kn{request\_memory}; per-slot job accounts are the fix if it matters \\
