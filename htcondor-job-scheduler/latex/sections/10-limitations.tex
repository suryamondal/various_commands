...
A GPU job running as \kn{nobody} & \textbf{Untested.} The startd enumerates the device because \kn{condor} was added to those groups; jobs run as \kn{nobody}, which is in none, so a real CUDA payload meets the same wall one layer down \\
